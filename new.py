...
# www.techrepublic.com/article/huffman-coding-in-python/
...
def get_codes(tree):
    ...
    def preorder(t, bits):
        if t.left:
            preorder(t.left, bits + "0")
        if t.right:
            preorder(t.right, bits + "1")
        if t.is_leaf():
            d[t.symbol] = bits
